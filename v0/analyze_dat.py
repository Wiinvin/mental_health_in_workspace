#!/usr/bin/env python
#
# file: $(EXP)/python/src/analyze_dat.py
#
# revision history:
#
...
def run_analysis(csv_file_a, params_a, odir_a):
    ...
    ## models later..
    #
    #target_events = ['age', 'gender', 'self_employed', 'no_employees', 'tech_company', 'remote_work', 'family_history']
    ...
    ## if categorical labels are requied
    #
    if categorical_f:
        ...
        x_tr, x_test, y_tr, y_test = train_test_split(dat_arr, lbl_arr,
                                                      stratify = lbl_arr,
                                                      test_size = 0.3,
                                                      random_state = DEF_RAND_STATE)
    ...
    ## Feature importance with the power of random initialization. We will run a small NN n times
    ## with different random seeds. The model will be convrged based on various
    ## initial points. Majority will focus on specific sets of features to optimize
    ## its performance. We zero padd each feature dimension after the model is
    ## trained and evaluate its performance. Model will perform poorly when certain
    ## indices are zero padded which carry the most information for this classification
    ## problem.
    
    ## create a neural network model and it's relevant parameters for analysis
    #
    total_mdl_iter = 40
    nn_init_seeds = [random.randint(7, 7777) for e in range(0, total_mdl_iter)] 

    accs, rcls, imp_feat_accs, imp_feat_rcls = feature_importance_via_nn(nn_init_seeds, x_tr, y_tr, x_test, y_test)

    ## separate low performing model's analysis from high performing model's performnace
    #
    low_performance_th = 0.5
    top_n_dims = 5
    feat_imp_mdls_acc = []
    feat_imp_mdls_rcl = []


    ## loop through accuracy and recall metrics
    #
    for i in range(len(accs)):

        if accs[i] < low_performance_th or rcls[i] < low_performance_th:

            ## deal with this later by finding linear combination on the relationship between the
            ## low performance and feature dimension's contribution
            #
            if len(imp_feat_accs[i]) > top_n_dims and len(imp_feat_rcls[i]) > top_n_dims:

                top_n_low_perf_festdims_acc = list(imp_feat_accs[i].values())
                top_n_low_acc_feats = top_n_low_perf_festdims_acc[:5]
                top_n_low_perf_festdims_rcl = list(imp_feat_rcls[i].values())
                top_n_low_rcl_feats = top_n_low_perf_festdims_rcl[:5]
        else:
            if len(imp_feat_accs[i]) > top_n_dims and len(imp_feat_rcls[i]) > top_n_dims:

                top_n_high_perf_festdims_acc = list(imp_feat_accs[i].values())
                top_n_high_acc_feats = top_n_high_perf_festdims_acc[:5]
                top_n_high_perf_festdims_rcl = list(imp_feat_rcls[i].values())
                top_n_high_rcl_feats = top_n_high_perf_festdims_rcl[:5]
                feat_imp_mdls_acc.append(top_n_high_acc_feats)
                feat_imp_mdls_rcl.append(top_n_high_rcl_feats)

        ## end of if
        #

            
    print (feat_imp_mdls_acc)
    print (feat_imp_mdls_rcl)

    ## list top N important features
    #
    ofile_acc = os.path.join(odir_a, "imp_feats_acc.txt")
    ofile_rcl = os.path.join(odir_a, "imp_feats_rcl.txt")    
    summarize_top_n_feats(feat_imp_mdls_acc, feat_names, ofile_acc)
    summarize_top_n_feats(feat_imp_mdls_rcl, feat_names, ofile_rcl)

    print("Analysis using neural network finished... ")
